...
router.post('/', function(req, res) {
  ...
  pool.connect(function(errorConnectingToDatabase, db, done) {
    if (errorConnectingToDatabase) {
      ...
      db.query('INSERT INTO "relations" ("user_id", "person_id", "mother_id", "father_id") VALUES ($1, $2, $3, $4) RETURNING id', [user_id, person_id, mother_id, father_id],
        function(err, result) {
          if (err) {
            console.log('Error making query!');
            res.sendStatus(500);
...
router.put('/', function(req, res) {
  ...
  pool.connect(function(errorConnectingToDatabase, db, done) {
    if (errorConnectingToDatabase) {
      ...
      db.query('UPDATE "relations" SET "mother_id" = $1, "father_id" = $2 WHERE "user_id"= $3 AND "person_id"= $4', [mother_id, father_id, user_id, person_id],
        function(err, result) {
          if (err) {
            console.log('Error making query!');
            res.sendStatus(500);
...
router.delete('/:id', function(req, res) {
  ...
  pool.connect(function(errorConnectingToDatabase, db, done) {
    if (errorConnectingToDatabase) {
      ...
      db.query('DELETE FROM "relations" WHERE id = $1', [id],
        function(err, result) {
          if (err) {
            console.log('Error making query');
            res.sendStatus(200);
